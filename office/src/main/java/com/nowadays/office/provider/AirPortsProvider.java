...
public class AirPortsProvider {
    ...
    public AirPort findAirPortAndRemoveBoard(String boardName){
        ...
        ports.stream().filter(airPort -> airPort.getBoards().contains(boardName)).findFirst().ifPresent(airPort -> {
    ...
    public AirPort getAirPort(String airPortName){
        return ports.stream().filter(airPort -> airPort.getName().equals(airPortName)).findFirst().orElse(new AirPort());
    }

    public RoutePoint getRoutPoint(String airPortName){
